#!/usr/bin/env python3
"""
Descarga el HTML de las páginas de resultados por temporada.
Lee scripts/temporadas.json y guarda cada HTML en data/raw/resultados/{temporada}.html
"""
...
OUTPUT_DIR = PROJECT_ROOT / "data" / "a_raw" / "resultados"
...
def main():
    ...
    for i, item in enumerate(temporadas, 1):
        ...
        url = item.get("url_resultados")
        if not url:
            print(f"  [{i}/{total}] {temporada}: sin url_resultados, omitido")
            continue
