...
CARPETA_RESULTADOS = (
    Path(__file__).resolve().parent.parent.parent / "data" / "raw" / "resultados"
)
CARPETA_STAGING = (
    Path(__file__).resolve().parent.parent.parent / "data" / "staging" / "resultados"
)
...
def extraer_tabla_resultados(html_path: Path) -> list[dict] | None:
    ...
    for row in table.find_all("tr", class_=lambda c: c and "jornadai" in c):
        ...
        goles_visitante = _parse_int(divs_goles[1].get_text(strip=True)) if len(divs_goles) > 1 else 0
